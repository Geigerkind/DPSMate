-- Global Variables
DPSMate.Modules.DetailsOverhealing = {}
...
function DPSMate.Modules.DetailsOverhealing:UpdateDetails(obj, key)
	...
	DPSMate_Details_Overhealing_SubTitle:SetText(DPSMate.L["activity"]..strformat("%.2f", (DPSMateCombatTime["effective"][key][obj.user] or 0)+1).."s "..DPSMate.L["of"].." "..strformat("%.2f", DPSMateCombatTime[mode[key]]).."s ("..strformat("%.2f", 100*((DPSMateCombatTime["effective"][key][obj.user] or 0)+1)/DPSMateCombatTime[mode[key]]).."%)")
	...
	DetailsArr, DetailsTotal, DmgArr = DPSMate.Modules.Overhealing:EvalTable(DPSMateUser[DetailsUser], curKey)
	...
	DPSMate_Details_Overhealing:SetScale((DPSMateSettings["targetscale"] or 0.58)/UIParent:GetScale())
end

function DPSMate.Modules.DetailsOverhealing:UpdateCompare(obj, key, comp)
	self:UpdateDetails(obj, key)
	
	DPSMate_Details_CompareOverhealing.proc = "None"
	...
	if not g4 then
		g4=DPSMate.Options.graph:CreateGraphPieChart("OHPieChartComp", DPSMate_Details_CompareOverhealing_Diagram, "CENTER", "CENTER", 0, 0, 200, 200)
		g5=DPSMate.Options.graph:CreateGraphLine("OHLineGraphComp",DPSMate_Details_CompareOverhealing_DiagramLine,"CENTER","CENTER",0,0,850,230)
		g6=DPSMate.Options.graph:CreateStackedGraph("OHStackedGraphComp",DPSMate_Details_CompareOverhealing_DiagramLine,"CENTER","CENTER",0,0,850,230)
		...
		g6:SetXLabels(true)
		g7=DPSMate.Options.graph:CreateGraphLine("OHLineGraphSum",DPSMate_Details_CompareOverhealing_Graph,"CENTER","CENTER",0,0,1750,230)
	end
	...
	DPSMate_Details_CompareOverhealing:Show()
	DPSMate_Details_CompareOverhealing_Graph:Show()
end

function DPSMate.Modules.DetailsOverhealing:UpdateSumGraph()
	...
	for cat, val in DPSMate:ScaleDown(sumTable, min) do
		tinsert(ata, {val[1],val[2], self:CheckProcs(DPSMate_Details_CompareOverhealing.proc, val[1]+min+1, DetailsUserComp)})
	end
	
	local Data2={{0,0}}
	for cat, val in DPSMate:ScaleDown(sumTableTwo, minT) do
		tinsert(Data2, {val[1],val[2], self:CheckProcs(DPSMate_Details_Overhealing.proc, val[1]+minT+1)})
	end

	g7:AddDataSeries(ata,{{0.2,0.8,0.2,0.8}, {0.5,0.8,0.9,0.8}}, self:AddProcPoints(DPSMate_Details_CompareOverhealing.proc, ata, DetailsUserComp))
	g7:AddDataSeries(Data2,{{1.0,0.0,0.0,0.8}, {1.0,1.0,0.0,0.8}}, self:AddProcPoints(DPSMate_Details_Overhealing.proc, Data2))
	g7:Show()
end
...
function DPSMate.Modules.DetailsOverhealing:ScrollFrame_Update(comp)
	if not comp then comp = DPSMate_Details_Overhealing.LastScroll or "" end
	local line, lineplusoffset
	local obj = _G("DPSMate_Details_"..comp.."Overhealing_Log_ScrollFrame")
	...
	for line=1,10 do
		...
		if uArr[lineplusoffset] ~= nil then
			...
			_G("DPSMate_Details_"..comp.."Overhealing_Log_ScrollButton"..line.."_Name"):SetText(ability)
			_G("DPSMate_Details_"..comp.."Overhealing_Log_ScrollButton"..line.."_Value"):SetText(dArr[lineplusoffset].." ("..strformat("%.2f", (dArr[lineplusoffset]*100/dTot)).."%)")
			_G("DPSMate_Details_"..comp.."Overhealing_Log_ScrollButton"..line.."_Icon"):SetTexture(DPSMate.BabbleSpell:GetSpellIcon(strsub(ability, 1, (strfind(ability, "%(") or 0)-1) or ability))
			if len < 10 then
				_G("DPSMate_Details_"..comp.."Overhealing_Log_ScrollButton"..line):SetWidth(235)
				_G("DPSMate_Details_"..comp.."Overhealing_Log_ScrollButton"..line.."_Name"):SetWidth(125)
			else
				_G("DPSMate_Details_"..comp.."Overhealing_Log_ScrollButton"..line):SetWidth(220)
				_G("DPSMate_Details_"..comp.."Overhealing_Log_ScrollButton"..line.."_Name"):SetWidth(110)
			end
			_G("DPSMate_Details_"..comp.."Overhealing_Log_ScrollButton"..line):Show()
		else
			_G("DPSMate_Details_"..comp.."Overhealing_Log_ScrollButton"..line):Hide()
		end
		_G("DPSMate_Details_"..comp.."Overhealing_Log_ScrollButton"..line.."_selected"):Hide()
		if dSel == lineplusoffset then
			_G("DPSMate_Details_"..comp.."Overhealing_Log_ScrollButton"..line.."_selected"):Show()
		end
	end
end

function DPSMate.Modules.DetailsOverhealing:SelectDetails_OverhealingButton(i, comp, cname)
	if not comp then comp = DPSMate_Details_Overhealing.LastScroll or "" end
	local pathh = ""
	...
	-- Hit
	_G("DPSMate_Details_"..comp.."Overhealing_LogDetails_Overhealing_Amount0_Amount"):SetText(hit)
	_G("DPSMate_Details_"..comp.."Overhealing_LogDetails_Overhealing_Amount0_Percent"):SetText(strformat("%.1f", 100*hit/total).."%")
	_G("DPSMate_Details_"..comp.."Overhealing_LogDetails_Overhealing_Amount0_StatusBar"):SetValue(ceil(100*hit/max))
	_G("DPSMate_Details_"..comp.."Overhealing_LogDetails_Overhealing_Amount0_StatusBar"):SetStatusBarColor(0.3,0.7,1.0,1)
	_G("DPSMate_Details_"..comp.."Overhealing_LogDetails_Overhealing_Average0"):SetText(ceil(hitav))
	_G("DPSMate_Details_"..comp.."Overhealing_LogDetails_Overhealing_Min0"):SetText(hitMin)
	_G("DPSMate_Details_"..comp.."Overhealing_LogDetails_Overhealing_Max0"):SetText(hitMax)
	
	-- Crit
	_G("DPSMate_Details_"..comp.."Overhealing_LogDetails_Overhealing_Amount1_Amount"):SetText(crit)
	_G("DPSMate_Details_"..comp.."Overhealing_LogDetails_Overhealing_Amount1_Percent"):SetText(strformat("%.1f", 100*crit/total).."%")
	_G("DPSMate_Details_"..comp.."Overhealing_LogDetails_Overhealing_Amount1_StatusBar"):SetValue(ceil(100*crit/max))
	_G("DPSMate_Details_"..comp.."Overhealing_LogDetails_Overhealing_Amount1_StatusBar"):SetStatusBarColor(1.0,0.7,0.3,1)
	_G("DPSMate_Details_"..comp.."Overhealing_LogDetails_Overhealing_Average1"):SetText(ceil(critav))
	_G("DPSMate_Details_"..comp.."Overhealing_LogDetails_Overhealing_Min1"):SetText(critMin)
	_G("DPSMate_Details_"..comp.."Overhealing_LogDetails_Overhealing_Max1"):SetText(critMax)
end

function DPSMate.Modules.DetailsOverhealing:Player_Update(comp)
	if not comp then comp = DPSMate_Details_Overhealing.LastScroll or "" end
	local line, lineplusoffset
	local path = "DPSMate_Details_"..comp.."Overhealing_player"
...
function DPSMate.Modules.DetailsOverhealing:PlayerSpells_Update(i, comp)
	if not comp then comp = DPSMate_Details_Overhealing.LastScroll or "" end
	local line, lineplusoffset
	local path = "DPSMate_Details_"..comp.."Overhealing_playerSpells"
	local obj = _G(path.."_ScrollFrame")
	obj.id = (i + FauxScrollFrame_GetOffset(_G("DPSMate_Details_"..comp.."Overhealing_player_ScrollFrame"))) or obj.id
	local d1,d2,d3,d4 = t1,t2,TTotal,PSelected
	if comp ~= "" and comp~=nil then
		d2 = t2Comp
		d4 = PSelected2
	...
	for p=1, 8 do
		_G("DPSMate_Details_"..comp.."Overhealing_player_ScrollButton"..p.."_selected"):Hide()
	end
	_G("DPSMate_Details_"..comp.."Overhealing_player_ScrollButton"..i.."_selected"):Show()
	if toggle3 then
		if toggle2 then
			if comp ~= "" and comp~=nil then
...
function DPSMate.Modules.DetailsOverhealing:UpdatePie(gg, cname)
...
function DPSMate.Modules.DetailsOverhealing:SortLineTable(t, b, cname)
	local newArr = {}
	if b then
...
function DPSMate.Modules.DetailsOverhealing:UpdateLineGraph(gg, comp, cname)
	if not comp then comp = DPSMate_Details_Overhealing.LastScroll or "" end
	if g3 then
		g3:Hide()
	end
	...
	local Data1={{0,0}}
	for cat, val in DPSMate:ScaleDown(sumTable, min) do
		tinsert(Data1, {val[1],val[2], DPSMate.Modules.DetailsOverhealing:CheckProcs(_G("DPSMate_Details_"..comp.."Overhealing").proc, val[1]+min+1, cname)})
	end
	...
	gg:AddDataSeries(Data1,colorT, self:AddProcPoints(_G("DPSMate_Details_"..comp.."Overhealing").proc, Data1, cname))
	gg:Show()
	toggle2=false
end

function DPSMate.Modules.DetailsOverhealing:UpdateStackedGraph(gg, comp, cname)
	if not comp then comp = DPSMate_Details_Overhealing.LastScroll or "" end
	if g2 then
		g2:Hide()
	end
...
function DPSMate.Modules.DetailsOverhealing:CreateGraphTable(comp)
	local lines = {}
	comp = comp or ""
	for i=1, 8 do
		-- Horizontal
		lines[i] = DPSMate.Options.graph:DrawLine(_G("DPSMate_Details_"..comp.."Overhealing_LogDetails_Overhealing"), 10, 270-i*30, 370, 270-i*30, 20, {0.5,0.5,0.5,0.5}, "BACKGROUND")
		lines[i]:Show()
	end
	-- Vertical
	lines[9] = DPSMate.Options.graph:DrawLine(_G("DPSMate_Details_"..comp.."Overhealing_LogDetails_Overhealing"), 57, 260, 57, 15, 20, {0.5,0.5,0.5,0.5}, "BACKGROUND")
	lines[9]:Show()
	
	lines[10] = DPSMate.Options.graph:DrawLine(_G("DPSMate_Details_"..comp.."Overhealing_LogDetails_Overhealing"), 192, 260, 192, 15, 20, {0.5,0.5,0.5,0.5}, "BACKGROUND")
	lines[10]:Show()
	
	lines[11] = DPSMate.Options.graph:DrawLine(_G("DPSMate_Details_"..comp.."Overhealing_LogDetails_Overhealing"), 252, 260, 252, 15, 20, {0.5,0.5,0.5,0.5}, "BACKGROUND")
	lines[11]:Show()
	
	lines[12] = DPSMate.Options.graph:DrawLine(_G("DPSMate_Details_"..comp.."Overhealing_LogDetails_Overhealing"), 312, 260, 312, 15, 20, {0.5,0.5,0.5,0.5}, "BACKGROUND")
	lines[12]:Show()
end
...
function DPSMate.Modules.DetailsOverhealing:ProcsDropDown()
    ...
    local function on_click()
        UIDropDownMenu_SetSelectedValue(DPSMate_Details_Overhealing_DiagramLegend_Procs, this.value)
		DPSMate_Details_Overhealing.proc = this.value
		if not toggle2 then
			DPSMate.Modules.DetailsOverhealing:UpdateLineGraph(g2, "")
		end
		if DetailsUserComp then
			DPSMate.Modules.DetailsOverhealing:UpdateSumGraph()
		end
    end
	...
	if DPSMate_Details_Overhealing.LastUser~=DetailsUser then
		UIDropDownMenu_SetSelectedValue(DPSMate_Details_Overhealing_DiagramLegend_Procs, "None")
	end
	DPSMate_Details_Overhealing.LastUser = DetailsUser
end

function DPSMate.Modules.DetailsOverhealing:ProcsDropDown_CompareOverhealing()
	local arr = DPSMate.Modules.DetailsOverhealing:GetAuraGainedArr(curKey)
	DPSMate_Details_CompareOverhealing.proc = "None"
	
    local function on_click()
        UIDropDownMenu_SetSelectedValue(DPSMate_Details_CompareOverhealing_DiagramLegend_Procs, this.value)
		DPSMate_Details_CompareOverhealing.proc = this.value
		if not toggle2 then
			DPSMate.Modules.DetailsOverhealing:UpdateLineGraph(g5, "Compare", DetailsUserComp)
		end
		if DetailsUserComp then
			DPSMate.Modules.DetailsOverhealing:UpdateSumGraph()
		end
	...
	if DPSMate_Details_CompareOverhealing.LastUser~=DetailsUserComp then
		UIDropDownMenu_SetSelectedValue(DPSMate_Details_CompareOverhealing_DiagramLegend_Procs, "None")
	end
	DPSMate_Details_CompareOverhealing.LastUser = DetailsUserComp
end

function DPSMate.Modules.DetailsOverhealing:GetSummarizedTable(arr, b, cname)
...
function DPSMate.Modules.DetailsOverhealing:CheckProcs(name, val, cname)
	local arr = DPSMate.Modules.DetailsOverhealing:GetAuraGainedArr(curKey)
...
function DPSMate.Modules.DetailsOverhealing:AddProcPoints(name, dat, cname)

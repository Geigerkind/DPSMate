...
function DPSMate.Modules.DetailsOverhealing:UpdateSumGraph()
	...
	for cat, val in DPSMate:ScaleDown(sumTable, min) do
		tinsert(ata, {val[1],val[2], self:CheckProcs(DPSMate_Details_CompareOverhealing.proc, val[1]+min, DetailsUserComp)})
	end
	
	local Data2={{0,0}}
	for cat, val in DPSMate:ScaleDown(sumTableTwo, minT) do
		tinsert(Data2, {val[1],val[2], self:CheckProcs(DPSMate_Details_Overhealing.proc, val[1]+minT)})
	end
...
function DPSMate.Modules.DetailsOverhealing:UpdateLineGraph(gg, comp, cname)
	...
	local Data1={{0,0}}
	for cat, val in DPSMate:ScaleDown(sumTable, min) do
		tinsert(Data1, {val[1],val[2], DPSMate.Modules.DetailsOverhealing:CheckProcs(_G("DPSMate_Details_"..comp.."Overhealing").proc, val[1]+min)})
	end
	...
	gg:AddDataSeries(Data1,colorT, self:AddProcPoints(_G("DPSMate_Details_"..comp.."Overhealing").proc, Data1))
	gg:Show()
	toggle2=false
end

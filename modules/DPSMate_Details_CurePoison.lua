		...
		if cat~="i" then
			...
			for ca, va in pairs(val) do
				...
				for c, v in pairs(va) do
					if DPSMate.Modules.CurePoison(DPSMate:GetAbilityById(c), DPSMate:GetAbilityById(cat)) then
						CVV = CVV + v
						local i = 1
						while true do
							if (not tbb[i]) then
...
function DPSMate.Modules.DetailsCurePoison:ScrollFrame_Update()
	...
	local obj = DPSMate_Details_CurePoison_Log_ScrollFrame
...
function DPSMate.Modules.DetailsCurePoison:SelectCreatureButton(i)
	local line, lineplusoffset
	local obj = DPSMate_Details_CurePoison_LogTwo_ScrollFrame
	i = i or obj.index
	obj.index = i
...
function DPSMate.Modules.DetailsCurePoison:SelectCreatureAbilityButton(i, p)
	local line, lineplusoffset
	local obj = DPSMate_Details_CurePoison_LogThree_ScrollFrame
	obj.index = i

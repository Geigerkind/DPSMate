...
function DPSMate.Modules.DetailsHealingTaken:SelectDetailsButton(p,i)
	local obj = DPSMate_Details_HealingTaken_Log_ScrollFrame
	...
	_G("DPSMate_Details_HealingTaken_LogDetails_Amount1_Percent"):SetText(strformat("%.1f", 100*hit/total).."%")
	...
	_G("DPSMate_Details_HealingTaken_LogDetails_Amount2_Percent"):SetText(strformat("%.1f", 100*crit/total).."%")
...
function DPSMate.Modules.DetailsHealingTaken:UpdateLineGraph()
	if not g2 then
		g2=DPSMate.Options.graph:CreateGraphLine("LineGraph",DPSMate_Details_HealingTaken_DiagramLine,"CENTER","CENTER",0,0,850,230)
	end
	if g then
		g:Hide()
	end
...
function DPSMate.Modules.DetailsHealingTaken:UpdateStackedGraph()
	if not g then
		g=DPSMate.Options.graph:CreateStackedGraph("StackedGraph",DPSMate_Details_HealingTaken_DiagramLine,"CENTER","CENTER",0,0,850,230)
...
function DPSMate.Modules.DetailsHealingTaken:CreateGraphTable()
	local lines = {}
	for i=1, 8 do
		-- Horizontal
		lines[i] = DPSMate.Options.graph:DrawLine(DPSMate_Details_HealingTaken_Log, 252, 270-i*30, 617, 270-i*30, 20, {0.5,0.5,0.5,0.5}, "BACKGROUND")
		lines[i]:Show()
	end
	-- Vertical
	lines[9] = DPSMate.Options.graph:DrawLine(DPSMate_Details_HealingTaken_Log, 302, 260, 302, 15, 20, {0.5,0.5,0.5,0.5}, "BACKGROUND")
	lines[9]:Show()
	
	lines[10] = DPSMate.Options.graph:DrawLine(DPSMate_Details_HealingTaken_Log, 437, 260, 437, 15, 20, {0.5,0.5,0.5,0.5}, "BACKGROUND")
	lines[10]:Show()
	
	lines[11] = DPSMate.Options.graph:DrawLine(DPSMate_Details_HealingTaken_Log, 497, 260, 497, 15, 20, {0.5,0.5,0.5,0.5}, "BACKGROUND")
	lines[11]:Show()
	
	lines[12] = DPSMate.Options.graph:DrawLine(DPSMate_Details_HealingTaken_Log, 557, 260, 557, 15, 20, {0.5,0.5,0.5,0.5}, "BACKGROUND")
	lines[12]:Show()
end

function DPSMate.Modules.DetailsHealingTaken:SortLineTable(t,b)
	local newArr = {}
	if b then
...
function DPSMate.Modules.DetailsHealingTaken:GetSummarizedTable(arr,b)

				...
				for c, v in pairs(va) do -- 3 Target
					if c==DPSMateUser[DetailsUser][1] then
						for ce, ve in pairs(v) do
							if DPSMate.Modules.Decurses:IsValid(DPSMate:GetAbilityById(ce), DPSMate:GetAbilityById(ca), DPSMateUser[DetailsUser]) then
...
function DPSMate.Modules.DetailsDecursesReceived:ScrollFrame_Update()
	...
	local obj = DPSMate_Details_DecursesReceived_Log_ScrollFrame
...
function DPSMate.Modules.DetailsDecursesReceived:SelectCreatureButton(i)
	local line, lineplusoffset
	local obj = DPSMate_Details_DecursesReceived_LogTwo_ScrollFrame
	i = i or obj.index
	obj.index = i
...
function DPSMate.Modules.DetailsDecursesReceived:SelectCreatureAbilityButton(i, p)
	local line, lineplusoffset
	local obj = DPSMate_Details_DecursesReceived_LogThree_ScrollFrame
	i = i or DPSMate_Details_DecursesReceived_LogTwo_ScrollFrame.index
	p = p or obj.index
	obj.index = p

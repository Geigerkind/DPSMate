...
function DPSMate.Modules.DetailsEHealingTaken:UpdateDetails(obj, key)
	DPSMate_Details_CompareEHealingTaken:Hide()
	DPSMate_Details_CompareEHealingTaken_Graph:Hide()
	...
	DPSMate_Details_EHealingTaken:SetScale((DPSMateSettings["targetscale"] or 0.58)/UIParent:GetScale())
end

function DPSMate.Modules.DetailsEHealingTaken:UpdateCompare(obj, key, comp)
	self:UpdateDetails(obj, key)
	
	DPSMate_Details_CompareEHealingTaken.proc = "None"
	--UIDropDownMenu_SetSelectedValue(DPSMate_Details_CompareEDD_DiagramLegend_Procs, "None")
	DetailsUserComp = comp
	DPSMate_Details_CompareEHealingTaken_Title:SetText(DPSMate.L["effhealtakenby"]..comp)
	...
	if not g3 then
		g3=DPSMate.Options.graph:CreateStackedGraph("EHTStackedGraphComp",DPSMate_Details_CompareEHealingTaken_DiagramLine,"CENTER","CENTER",0,0,850,230)
		...
		g3:SetXLabels(true)
		g4=DPSMate.Options.graph:CreateGraphLine("EHTLineGraphComp",DPSMate_Details_CompareEHealingTaken_DiagramLine,"CENTER","CENTER",0,0,850,230)
		g7=DPSMate.Options.graph:CreateGraphLine("EHTGraphSumComp",DPSMate_Details_CompareEHealingTaken_Graph,"CENTER","CENTER",0,0,1750,230)
	end
	...
	DPSMate_Details_CompareEHealingTaken:Show()
	DPSMate_Details_CompareEHealingTaken_Graph:Show()
end

function DPSMate.Modules.DetailsEHealingTaken:UpdateSumGraph()
...
function DPSMate.Modules.DetailsEHealingTaken:ScrollFrame_Update(comp)
	if not comp then comp = DPSMate_Details_EHealingTaken.LastScroll or "" end
	local line, lineplusoffset
	local path = "DPSMate_Details_"..comp.."EHealingTaken_LogCreature"
...
function DPSMate.Modules.DetailsEHealingTaken:SelectCreatureButton(i, comp)
	if not comp then comp = DPSMate_Details_EHealingTaken.LastScroll or "" end
	local line, lineplusoffset
	local path = "DPSMate_Details_"..comp.."EHealingTaken_Log"
	local obj = _G(path.."_ScrollFrame")
	local uArr, dArr, dTot, dSel = DetailsArr, DmgArr, DetailsTotal, DetailsSelected
	if comp ~= "" and comp~=nil then
		uArr = DetailsArrComp
		dArr = DmgArrComp
		dTot = DetailsTotalComp
		DetailsSelectedComp = i+FauxScrollFrame_GetOffset(_G("DPSMate_Details_"..comp.."EHealingTaken_LogCreature_ScrollFrame"))
		dSel = DetailsSelectedComp
	else
		DetailsSelected = i+FauxScrollFrame_GetOffset(_G("DPSMate_Details_"..comp.."EHealingTaken_LogCreature_ScrollFrame"))
	...
	for p=1, 10 do
		_G("DPSMate_Details_"..comp.."EHealingTaken_LogCreature_ScrollButton"..p.."_selected"):Hide()
	end
	_G("DPSMate_Details_"..comp.."EHealingTaken_LogCreature_ScrollButton"..i.."_selected"):Show()
...
function DPSMate.Modules.DetailsEHealingTaken:SelectDetailsButton(p,i, comp, cname)
	if not comp then comp = DPSMate_Details_EHealingTaken.LastScroll or "" end
	local obj = _G("DPSMate_Details_"..comp.."EHealingTaken_Log_ScrollFrame")
	local lineplusoffset = i + FauxScrollFrame_GetOffset(obj)
	local uArr, dArr, dSel = DetailsArr, DmgArr, DetailsSelected
	
	for p=1, 10 do
		_G("DPSMate_Details_"..comp.."EHealingTaken_Log_ScrollButton"..p.."_selected"):Hide()
	end
	...
	_G("DPSMate_Details_"..comp.."EHealingTaken_Log_ScrollButton"..i.."_selected"):Show()
	...
	-- Hit
	_G("DPSMate_Details_"..comp.."EHealingTaken_LogDetails_Amount1_Amount"):SetText(hit)
	...
	-- Crit
	_G("DPSMate_Details_"..comp.."EHealingTaken_LogDetails_Amount2_Amount"):SetText(crit)
...
function DPSMate.Modules.DetailsEHealingTaken:UpdateLineGraph(gg, comp, cname)
	if g then
		g:Hide()
	end
	if g3 then
...
function DPSMate.Modules.DetailsEHealingTaken:UpdateStackedGraph(gg, comp, cname)
	if g2 then
		g2:Hide()
	end
	if g4 then
...
function DPSMate.Modules.DetailsEHealingTaken:CreateGraphTable(comp)
	local lines = {}
	comp = comp or ""
	for i=1, 8 do
		-- Horizontal
		lines[i] = DPSMate.Options.graph:DrawLine(_G("DPSMate_Details_"..comp.."EHealingTaken_Log"), 252, 270-i*30, 617, 270-i*30, 20, {0.5,0.5,0.5,0.5}, "BACKGROUND")
		lines[i]:Show()
	end
	-- Vertical
	lines[9] = DPSMate.Options.graph:DrawLine(_G("DPSMate_Details_"..comp.."EHealingTaken_Log"), 302, 260, 302, 15, 20, {0.5,0.5,0.5,0.5}, "BACKGROUND")
	lines[9]:Show()
	
	lines[10] = DPSMate.Options.graph:DrawLine(_G("DPSMate_Details_"..comp.."EHealingTaken_Log"), 437, 260, 437, 15, 20, {0.5,0.5,0.5,0.5}, "BACKGROUND")
	lines[10]:Show()
	
	lines[11] = DPSMate.Options.graph:DrawLine(_G("DPSMate_Details_"..comp.."EHealingTaken_Log"), 497, 260, 497, 15, 20, {0.5,0.5,0.5,0.5}, "BACKGROUND")
	lines[11]:Show()
	
	lines[12] = DPSMate.Options.graph:DrawLine(_G("DPSMate_Details_"..comp.."EHealingTaken_Log"), 557, 260, 557, 15, 20, {0.5,0.5,0.5,0.5}, "BACKGROUND")
	lines[12]:Show()
end
...
function DPSMate.Modules.DetailsEHealingTaken:GetSummarizedTable(arr,b, cname)

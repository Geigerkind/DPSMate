...
function DPSMate.Modules.DetailsDecurses:UpdateDetails(obj, key)
	...
	DPSMate_Details_Decurses_Title:SetText("Decurses by "..obj.user)
	DPSMate_Details_Decurses:Show()
	DPSMate.Modules.DetailsDecurses:ScrollFrame_Update()
	DPSMate.Modules.DetailsDecurses:SelectCreatureButton(1)
	DPSMate.Modules.DetailsDecurses:SelectCreatureAbilityButton(1,1)
end
		...
		if cat~="i" then
			...
			for ca, va in pairs(val) do
				...
				for c, v in pairs(va) do
					if DPSMateAbility[DPSMate:GetAbilityById(c)][2]=="Curse" then -- Performance can be increased here
						CVV = CVV + v
						local i = 1
						while true do
							if (not tbb[i]) then
...
function DPSMate.Modules.DetailsDecurses:ScrollFrame_Update()
	...
	local obj = getglobal("DPSMate_Details_Decurses_Log_ScrollFrame")
...
function DPSMate.Modules.DetailsDecurses:SelectCreatureButton(i)
	local line, lineplusoffset
	local obj = getglobal("DPSMate_Details_Decurses_LogTwo_ScrollFrame")
	i = i or obj.index
	obj.index = i
	...
	for p=1, 14 do
		getglobal("DPSMate_Details_Decurses_Log_ScrollButton"..p.."_selected"):Hide()
	end
	getglobal(path.."1_selected"):Show()
	DPSMate.Modules.DetailsDecurses:SelectCreatureAbilityButton(i, 1)
	getglobal("DPSMate_Details_Decurses_Log_ScrollButton"..i.."_selected"):Show()
end

function DPSMate.Modules.DetailsDecurses:SelectCreatureAbilityButton(i, p)
	local line, lineplusoffset
	local obj = getglobal("DPSMate_Details_Decurses_LogThree_ScrollFrame")
	i = i or getglobal("DPSMate_Details_Decurses_LogTwo_ScrollFrame").index
	p = p or obj.index
	obj.index = p
	...
	for i=1, 14 do
		getglobal("DPSMate_Details_Decurses_LogTwo_ScrollButton"..i.."_selected"):Hide()
	end
	getglobal("DPSMate_Details_Decurses_LogTwo_ScrollButton"..p.."_selected"):Show()
end
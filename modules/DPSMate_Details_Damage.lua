...
function DPSMate.Modules.DetailsDamage:ScrollFrame_Update()
	...
	local obj = DPSMate_Details_Log_ScrollFrame
...
function DPSMate.Modules.DetailsDamage:PlayerSpells_Update(i)
	...
	for line=1,10 do
		...
		if t2[obj.id][2][lineplusoffset] ~= nil then
			...
			_G(path.."_ScrollButton"..line.."_Value"):SetText(t2[obj.id][3][lineplusoffset][13].." ("..strformat("%.2f", (t2[obj.id][3][lineplusoffset][13]*100/t2[obj.id][1])).."%)")
			_G(path.."_ScrollButton"..line.."_Icon"):SetTexture(DPSMate.BabbleSpell:GetSpellIcon(strsub(ability, 1, (strfind(ability, "%(") or 0)-1) or ability))
			if len < 10 then
				_G(path.."_ScrollButton"..line):SetWidth(235)
				_G(path.."_ScrollButton"..line.."_Name"):SetWidth(125)
...
function DPSMate.Modules.DetailsDamage:SelectDetailsButton(i)
	...
	_G("DPSMate_Details_LogDetails_Amount0_Percent"):SetText(strformat("%.1f", 100*block/total).."%")
	...
	_G("DPSMate_Details_LogDetails_Amount1_Percent"):SetText(strformat("%.1f", 100*glance/total).."%")
	...
	_G("DPSMate_Details_LogDetails_Amount2_Percent"):SetText(strformat("%.1f", 100*hit/total).."%")
	...
	_G("DPSMate_Details_LogDetails_Amount3_Percent"):SetText(strformat("%.1f", 100*crit/total).."%")
	...
	_G("DPSMate_Details_LogDetails_Amount4_Percent"):SetText(strformat("%.1f", 100*miss/total).."%")
	...
	_G("DPSMate_Details_LogDetails_Amount5_Percent"):SetText(strformat("%.1f", 100*parry/total).."%")
	...
	_G("DPSMate_Details_LogDetails_Amount6_Percent"):SetText(strformat("%.1f", 100*dodge/total).."%")
	...
	_G("DPSMate_Details_LogDetails_Amount7_Percent"):SetText(strformat("%.1f", 100*resist/total).."%")
...
function DPSMate.Modules.DetailsDamage:UpdateLineGraph()
	...
	for cat, val in DPSMate:ScaleDown(sumTable, min) do
		tinsert(Data1, {val[1],val[2], self:CheckProcs(DPSMate_Details.proc, val[1]+min)})
	end
...
function DPSMate.Modules.DetailsDamage:SortLineTable(t, b)
	local newArr = {}
	if b then
		...
		if DPSMateUser[DetailsUser][5] then
			if t[DPSMateUser[DPSMateUser[DetailsUser][5]][1]] then
				for cat, val in t[b][DPSMateUser[DPSMateUser[DetailsUser][5]][1]] do
					if cat~="i" and val["i"] then
						for ca, va in val["i"] do
							local i=1
							while true do
...
function DPSMate.Modules.DetailsDamage:GetSummarizedTable(arr, b)

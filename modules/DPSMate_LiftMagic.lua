		...
		if cat~="i" then
			for ca, va in pairs(val) do -- 3 Target
				for c, v in pairs(va) do -- 10 Cured Ability
					if DPSMateAbility[DPSMate:GetAbilityById(c)][2]=="Magic" then
...
function DPSMate.Modules.LiftMagic:GetSettingValues(arr, cbt, k)
	...
	sortedTable, total, a = DPSMate.Modules.LiftMagic:GetSortedTable(arr)
...
function DPSMate.Modules.LiftMagic:ShowTooltip(user,k)
	local a,b,c = DPSMate.Modules.LiftMagic:EvalTable(DPSMateUser[user], k)
...
function DPSMate.Modules.LiftMagic:OpenDetails(obj, key)

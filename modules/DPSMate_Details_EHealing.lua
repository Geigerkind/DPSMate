...
function DPSMate.Modules.DetailsEHealing:UpdateCompare(obj, key, comp)
	...
	DPSMate_Details_CompareEHealing_Title:SetText(DPSMate.L["dmgdoneby"]..comp)
...
function DPSMate.Modules.DetailsEHealing:UpdateSumGraph()
	...
	for cat, val in DPSMate:ScaleDown(sumTable, min) do
		tinsert(ata, {val[1],val[2], self:CheckProcs(DPSMate_Details_CompareEHealing.proc, val[1]+min, DetailsUserComp)})
	end
	
	local Data2={{0,0}}
	for cat, val in DPSMate:ScaleDown(sumTableTwo, minT) do
		tinsert(Data2, {val[1],val[2], self:CheckProcs(DPSMate_Details_EHealing.proc, val[1]+minT)})
	end
...
function DPSMate.Modules.DetailsEHealing:UpdateLineGraph(gg, comp, cname)
	...
	local Data1={{0,0}}
	for cat, val in DPSMate:ScaleDown(sumTable, min) do
		tinsert(Data1, {val[1],val[2], DPSMate.Modules.DetailsEHealing:CheckProcs(_G("DPSMate_Details_"..comp.."EHealing").proc, val[1]+min)})
	end
	...
	gg:AddDataSeries(Data1,colorT, self:AddProcPoints(_G("DPSMate_Details_"..comp.."EHealing").proc, Data1))
	gg:Show()
	toggle2=false
end

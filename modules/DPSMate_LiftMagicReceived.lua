...
DPSMate.Options.Options[1]["args"]["liftmagicreceived"] = {
	order = 225,
	type = 'toggle',
	name = 'Magic lift received',
	desc = "Show Magic lift received.",
...
DPSMate:Register("liftmagicreceived", DPSMate.Modules.LiftMagicReceived, "Magic lifts received")


function DPSMate.Modules.LiftMagicReceived:GetSortedTable(arr)
...
function DPSMate.Modules.LiftMagicReceived:GetSettingValues(arr, cbt, k)
	...
	sortedTable, total, a = DPSMate.Modules.LiftMagicReceived:GetSortedTable(arr)
	for cat, val in pairs(sortedTable) do
		local dmg, tot, sort = DPSMate:FormatNumbers(val, total, sortedTable[1], k)
		if dmg==0 then break end
		local str = {[1]="",[2]="",[3]=""}
		if DPSMateSettings["columnsmagicreceived"][1] then str[1] = " "..dmg..p; strt[2] = tot..p end
		if DPSMateSettings["columnsmagicreceived"][2] then str[3] = " ("..string.format("%.1f", 100*dmg/tot).."%)" end

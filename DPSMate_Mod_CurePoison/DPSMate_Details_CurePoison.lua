...
function DPSMate.Modules.DetailsCurePoison:UpdateDetails(obj, key)
	DPSMate_Details_CompareCurePoison:Hide()
	
	curKey = key
	db, cbt = DPSMate:GetMode(key)
	DetailsUser = obj.user
	DetailsUserComp = nil
	DPSMate_Details_CurePoison_Title:SetText(DPSMate.L["poisoncuredby"]..obj.user)
	...
	DPSMate_Details_CurePoison:SetScale((DPSMateSettings["targetscale"] or 0.58)/UIParent:GetScale())
end

function DPSMate.Modules.DetailsCurePoison:UpdateCompare(obj, key, comp)
	self:UpdateDetails(obj, key)

	DetailsUserComp = comp
	DPSMate_Details_CompareCurePoison_Title:SetText(DPSMate.L["poisoncuredby"]..comp)
	DetailsArrComp, DetailsTotalComp, DmgArrComp = self:EvalTable(comp)
	DPSMate_Details_CompareCurePoison:Show()
	self:ScrollFrame_Update("Compare")
	self:SelectCreatureButton(1,"Compare")
	self:SelectCreatureAbilityButton(1,1,"Compare")
end

function DPSMate.Modules.DetailsCurePoison:EvalTable(cname)
	local a, b, total = {}, {}, 0
	for cat, val in db[DPSMateUser[cname or DetailsUser][1]] do -- 41 Ability
		if cat~="i" then
			...
			for ca, va in val do
				local taa, tbb, CVV = {}, {}, 0
				for c, v in va do
					if DPSMate.Modules.CurePoison:IsValid(DPSMate:GetAbilityById(c), DPSMate:GetAbilityById(cat)) then
						CVV = CVV + v
						local i = 1
						while true do
							if (not tbb[i]) then
...
function DPSMate.Modules.DetailsCurePoison:ScrollFrame_Update(comp)
	comp = comp or DPSMate_Details_CurePoison.LastScroll
	local line, lineplusoffset
	local obj = _G("DPSMate_Details_"..comp.."CurePoison_Log_ScrollFrame")
	local path = "DPSMate_Details_"..comp.."CurePoison_Log_ScrollButton"
...
function DPSMate.Modules.DetailsCurePoison:SelectCreatureButton(i, comp)
	comp = comp or DPSMate_Details_CurePoison.LastScroll
	local line, lineplusoffset
	local obj = _G("DPSMate_Details_"..comp.."CurePoison_LogTwo_ScrollFrame")
	i = i or obj.index
	obj.index = i
	local path = "DPSMate_Details_"..comp.."CurePoison_LogTwo_ScrollButton"
	local uArr, dArr, dTot = DetailsArr, DmgArr, DetailsTotal
	if comp~="" and comp then
		uArr = DetailsArrComp
		dArr = DmgArrComp
	...
	for p=1, 14 do
		_G("DPSMate_Details_"..comp.."CurePoison_Log_ScrollButton"..p.."_selected"):Hide()
	end
	_G(path.."1_selected"):Show()
	DPSMate.Modules.DetailsCurePoison:SelectCreatureAbilityButton(i, 1, comp)
	_G("DPSMate_Details_"..comp.."CurePoison_Log_ScrollButton"..i.."_selected"):Show()
end

function DPSMate.Modules.DetailsCurePoison:SelectCreatureAbilityButton(i, p, comp)
	comp = comp or DPSMate_Details_CurePoison.LastScroll
	local line, lineplusoffset
	local obj = _G("DPSMate_Details_"..comp.."CurePoison_LogThree_ScrollFrame")
	i = i or _G("DPSMate_Details_"..comp.."CurePoison_LogTwo_ScrollFrame").index
	p = p or obj.index
	obj.index = p
	local path = "DPSMate_Details_"..comp.."CurePoison_LogThree_ScrollButton"
	local uArr, dArr, dTot = DetailsArr, DmgArr, DetailsTotal
	if comp~="" and comp then
		uArr = DetailsArrComp
		dArr = DmgArrComp
	...
	for i=1, 14 do
		_G("DPSMate_Details_"..comp.."CurePoison_LogTwo_ScrollButton"..i.."_selected"):Hide()
	end
	_G("DPSMate_Details_"..comp.."CurePoison_LogTwo_ScrollButton"..p.."_selected"):Show()
end
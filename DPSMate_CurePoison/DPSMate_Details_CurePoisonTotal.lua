-- Global Variables
DPSMate.Modules.DetailsCurePoisonTotal = {}
...
function DPSMate.Modules.DetailsCurePoisonTotal:UpdateDetails(obj, key)
	...
	DPSMate_Details_CurePoison_Total_Title:SetText(DPSMate.L["curepoisonsum"])
	DPSMate_Details_CurePoison_Total:Show()
	DPSMate_Details_CurePoison_Total:SetScale((DPSMateSettings["targetscale"] or 0.58)/UIParent:GetScale())
end

function DPSMate.Modules.DetailsCurePoisonTotal:UpdateRow(row, a, b, c, d, e)
	_G("DPSMate_Details_CurePoison_Total_LogDetails_Row"..row).user = c
	_G("DPSMate_Details_CurePoison_Total_LogDetails_Row"..row.."_Time"):SetText(a or "Unknown")
	if b~=0 then
		_G("DPSMate_Details_CurePoison_Total_LogDetails_Row"..row.."_CombatTime"):SetText(strformat("%.2f", b).."s")
	else
...
function DPSMate.Modules.DetailsCurePoisonTotal:EvalTable()
	local a = {}
	for cat, val in db do -- user
		...
		if DPSMate:ApplyFilter(curKey, name) then
			for ca, va in val["i"][2] do
				...
				if DPSMate.Modules.CurePoison:IsValid(ab, abtwo) then
					local i=1
					while true do
						if not a[i] then
...
function DPSMate.Modules.DetailsCurePoisonTotal:ScrollFrameUpdate(direction)
...
function DPSMate.Modules.DetailsCurePoisonTotal:CreateGraphTable(obj)
	local lines = {}
	comp = comp or ""
	for i=1, 11 do
		-- Horizontal

...
function DPSMate.Modules.DetailsOHealingTaken:UpdateDetails(obj, key)
	DPSMate_Details_CompareOHealingTaken:Hide()
	DPSMate_Details_CompareOHealingTaken_Graph:Hide()
	
	curKey = key
	db, cbt = DPSMate:GetMode(key)
	DetailsUser = obj.user
	DetailsUserComp = nil
	DPSMate_Details_OHealingTaken_Title:SetText(DPSMate.L["ohealtakenby"]..obj.user)
	DetailsArr, DetailsTotal, DmgArr = DPSMate.RegistredModules[DPSMateSettings["windows"][curKey]["CurMode"]]:EvalTable(DPSMateUser[DetailsUser], curKey)
	DPSMate_Details_OHealingTaken:Show()
	...
	DPSMate_Details_OHealingTaken:SetScale((DPSMateSettings["targetscale"] or 0.58)/UIParent:GetScale())
end

function DPSMate.Modules.DetailsOHealingTaken:UpdateCompare(obj, key, comp)
	self:UpdateDetails(obj, key)
	
	DPSMate_Details_CompareOHealingTaken.proc = "None"
	--UIDropDownMenu_SetSelectedValue(DPSMate_Details_CompareEDD_DiagramLegend_Procs, "None")
	DetailsUserComp = comp
	DPSMate_Details_CompareOHealingTaken_Title:SetText(DPSMate.L["ohealtakenby"]..comp)
	...
	if not g3 then
		g3=DPSMate.Options.graph:CreateStackedGraph("OHTStackedGraphComp",DPSMate_Details_CompareOHealingTaken_DiagramLine,"CENTER","CENTER",0,0,850,230)
		...
		g3:SetXLabels(true)
		g4=DPSMate.Options.graph:CreateGraphLine("OHTLineGraphComp",DPSMate_Details_CompareOHealingTaken_DiagramLine,"CENTER","CENTER",0,0,850,230)
		g7=DPSMate.Options.graph:CreateGraphLine("OHTGraphSumComp",DPSMate_Details_CompareOHealingTaken_Graph,"CENTER","CENTER",0,0,1750,230)
	end
	...
	DPSMate_Details_CompareOHealingTaken:Show()
	DPSMate_Details_CompareOHealingTaken_Graph:Show()
end

function DPSMate.Modules.DetailsOHealingTaken:UpdateSumGraph()
...
function DPSMate.Modules.DetailsOHealingTaken:ScrollFrame_Update(comp)
	if not comp then comp = DPSMate_Details_OHealingTaken.LastScroll or "" end
	local line, lineplusoffset
	local path = "DPSMate_Details_"..comp.."OHealingTaken_LogCreature"
...
function DPSMate.Modules.DetailsOHealingTaken:SelectCreatureButton(i, comp)
	if not comp then comp = DPSMate_Details_OHealingTaken.LastScroll or "" end
	local line, lineplusoffset
	local path = "DPSMate_Details_"..comp.."OHealingTaken_Log"
	local obj = _G(path.."_ScrollFrame")
	local uArr, dArr, dTot, dSel = DetailsArr, DmgArr, DetailsTotal, DetailsSelected
	if comp ~= "" and comp~=nil then
		uArr = DetailsArrComp
		dArr = DmgArrComp
		dTot = DetailsTotalComp
		DetailsSelectedComp = i+FauxScrollFrame_GetOffset(_G("DPSMate_Details_"..comp.."OHealingTaken_LogCreature_ScrollFrame"))
		dSel = DetailsSelectedComp
	else
		DetailsSelected = i+FauxScrollFrame_GetOffset(_G("DPSMate_Details_"..comp.."OHealingTaken_LogCreature_ScrollFrame"))
	...
	for p=1, 10 do
		_G("DPSMate_Details_"..comp.."OHealingTaken_LogCreature_ScrollButton"..p.."_selected"):Hide()
	end
	_G("DPSMate_Details_"..comp.."OHealingTaken_LogCreature_ScrollButton"..i.."_selected"):Show()
...
function DPSMate.Modules.DetailsOHealingTaken:SelectDetailsButton(p,i, comp, cname)
	if not comp then comp = DPSMate_Details_OHealingTaken.LastScroll or "" end
	local obj = _G("DPSMate_Details_"..comp.."OHealingTaken_Log_ScrollFrame")
	local lineplusoffset = i + FauxScrollFrame_GetOffset(obj)
	local uArr, dArr, dSel = DetailsArr, DmgArr, DetailsSelected
	
	for p=1, 10 do
		_G("DPSMate_Details_"..comp.."OHealingTaken_Log_ScrollButton"..p.."_selected"):Hide()
	end
	...
	_G("DPSMate_Details_"..comp.."OHealingTaken_Log_ScrollButton"..i.."_selected"):Show()
	...
	-- Hit
	_G("DPSMate_Details_"..comp.."OHealingTaken_LogDetails_Amount1_Amount"):SetText(hit)
	_G("DPSMate_Details_"..comp.."OHealingTaken_LogDetails_Amount1_Percent"):SetText(strformat("%.1f", 100*hit/total).."%")
	_G("DPSMate_Details_"..comp.."OHealingTaken_LogDetails_Amount1_StatusBar"):SetValue(ceil(100*hit/max))
	_G("DPSMate_Details_"..comp.."OHealingTaken_LogDetails_Amount1_StatusBar"):SetStatusBarColor(1.0,0.7,0.3,1)
	_G("DPSMate_Details_"..comp.."OHealingTaken_LogDetails_Average1"):SetText(ceil(hitav/DPSMate:ProbZero(hit)))
	_G("DPSMate_Details_"..comp.."OHealingTaken_LogDetails_Min1"):SetText(hitMin)
	_G("DPSMate_Details_"..comp.."OHealingTaken_LogDetails_Max1"):SetText(hitMax)
	
	-- Crit
	_G("DPSMate_Details_"..comp.."OHealingTaken_LogDetails_Amount2_Amount"):SetText(crit)
	_G("DPSMate_Details_"..comp.."OHealingTaken_LogDetails_Amount2_Percent"):SetText(strformat("%.1f", 100*crit/total).."%")
	_G("DPSMate_Details_"..comp.."OHealingTaken_LogDetails_Amount2_StatusBar"):SetValue(ceil(100*crit/max))
	_G("DPSMate_Details_"..comp.."OHealingTaken_LogDetails_Amount2_StatusBar"):SetStatusBarColor(0.9,0.0,0.0,1)
	_G("DPSMate_Details_"..comp.."OHealingTaken_LogDetails_Average2"):SetText(ceil(critav/DPSMate:ProbZero(crit)))
	_G("DPSMate_Details_"..comp.."OHealingTaken_LogDetails_Min2"):SetText(critMin)
	_G("DPSMate_Details_"..comp.."OHealingTaken_LogDetails_Max2"):SetText(critMax)
end

function DPSMate.Modules.DetailsOHealingTaken:UpdateLineGraph(gg, comp, cname)
	if g then
		g:Hide()
	end
	if g3 then
...
function DPSMate.Modules.DetailsOHealingTaken:UpdateStackedGraph(gg, comp, cname)
	if g2 then
		g2:Hide()
	end
	if g4 then
...
function DPSMate.Modules.DetailsOHealingTaken:CreateGraphTable(comp)
	local lines = {}
	comp = comp or ""
	for i=1, 8 do
		-- Horizontal
		lines[i] = DPSMate.Options.graph:DrawLine(_G("DPSMate_Details_"..comp.."OHealingTaken_Log"), 252, 270-i*30, 617, 270-i*30, 20, {0.5,0.5,0.5,0.5}, "BACKGROUND")
		lines[i]:Show()
	end
	-- Vertical
	lines[9] = DPSMate.Options.graph:DrawLine(_G("DPSMate_Details_"..comp.."OHealingTaken_Log"), 302, 260, 302, 15, 20, {0.5,0.5,0.5,0.5}, "BACKGROUND")
	lines[9]:Show()
	
	lines[10] = DPSMate.Options.graph:DrawLine(_G("DPSMate_Details_"..comp.."OHealingTaken_Log"), 437, 260, 437, 15, 20, {0.5,0.5,0.5,0.5}, "BACKGROUND")
	lines[10]:Show()
	
	lines[11] = DPSMate.Options.graph:DrawLine(_G("DPSMate_Details_"..comp.."OHealingTaken_Log"), 497, 260, 497, 15, 20, {0.5,0.5,0.5,0.5}, "BACKGROUND")
	lines[11]:Show()
	
	lines[12] = DPSMate.Options.graph:DrawLine(_G("DPSMate_Details_"..comp.."OHealingTaken_Log"), 557, 260, 557, 15, 20, {0.5,0.5,0.5,0.5}, "BACKGROUND")
	lines[12]:Show()
end

function DPSMate.Modules.DetailsOHealingTaken:SortLineTable(t,b, cname)
	local newArr = {}
	if b then
...
function DPSMate.Modules.DetailsOHealingTaken:GetSummarizedTable(arr,b, cname)
	return DPSMate.Sync:GetSummarizedTable(self:SortLineTable(arr,b,cname))
end

function DPSMate.Modules.DetailsOHealingTaken:ToggleMode() 
	if toggle then
		self:UpdateLineGraph(g2, "")
		if DetailsUserComp then
			self:UpdateLineGraph(g4, "Compare", DetailsUserComp)
...
function DPSMate.Modules.DetailsOHealingTaken:ToggleIndividual()
	if toggle3 then
		toggle3 = false
	else
		toggle3 = true

...
DPSMate.Modules.OHealingTaken.Events = {
...
function DPSMate.Modules.OHealingTaken:GetSettingValues(arr, cbt, k,ecbt)
	...
	for cat, val in pairs(sortedTable) do
		...
		if DPSMateSettings["columnsohealingtaken"][1] then str[1] = " "..DPSMate:Commas(va, k)..p; strt[2] = DPSMate:Commas(tot, k)..pt end
		if DPSMateSettings["columnsohealingtaken"][3] then str[2] = " ("..strformat("%.1f", 100*varea/totr).."%)" end
		if DPSMateSettings["columnsohealingtaken"][2] then str[3] = " ("..strformat("%.1f", va/cbt)..")"; strt[1] = " ("..strformat("%.1f", (tot/cbt))..pt..")" end
